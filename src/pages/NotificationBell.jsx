      ...
      <AnimatePresence>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-center mb-1">
                        <h4 className={`text-xs font-bold ${!n.read ? 'text-gray-900 dark:text-white' : 'text-gray-400'}`}>{t(`notifications.${n.titleKey}`, { ns: 'common' })}</h4>
                        <span className="text-[10px] text-gray-400">{t(`notifications.${n.timeKey}`, { ns: 'common' })}</span>
                      </div>
                      <p className="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed">{t(`notifications.${n.descKey}`, { ns: 'common' })}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {selectedNotif && (
          ...
          >
            ...
            <h2 className="text-base font-bold text-gray-900 dark:text-white mb-2">{t(`notifications.${selectedNotif.titleKey}`, { ns: 'common' })}</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed mb-8">{t(`notifications.${selectedNotif.detailKey}`, { ns: 'common' })}</p>
            <button className="w-full py-3 bg-blue-600 rounded-xl text-sm font-bold text-white hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20">{t('notifications.viewDocument')}</button>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
