...
export default function SearchPapers() {
  ...
  const filteredPapers = PAPERS.filter((p) => {
    ...
    if (currentRole === 'researcher') {
      if (filters.startYear && p.year < parseInt(filters.startYear, 10))
        return false;
      if (filters.endYear && p.year > parseInt(filters.endYear, 10))
        return false;
      if (filters.fields.length > 0 && !filters.fields.includes(p.field))
        return false;
      if (
        filters.minCitations &&
        p.citations < parseInt(filters.minCitations, 10)
      )
        return false;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pt-2 items-start">
        ...
        <div className="md:col-span-3 space-y-3">
          <p className="text-xs text-slate-500 pl-1">
            ...
            {filteredPapers.length === 1 ? 'paper' : 'papers'} matching your
            criteria
          </p>
