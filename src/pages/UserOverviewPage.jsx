...
const StatCard = ({ label, value, change, Icon, accent }) => (
 ...
 <div className="flex items-start justify-between mb-2">
  ...
  <span
  className="text-xs font-bold px-2 py-1 rounded-md bg-white/[0.04]"
  style={{ color: change.startsWith('+') ? '#34D399' : '#EF4444' }}
  >
  {change}
  </span>
 </div>
...
export default function UserOverviewPage() {
 ...
 <div className="w-full h-full min-h-screen p-8 space-y-6 overflow-y-auto bg-transparent ">
  ...
  <div>
   <h2 className="text-2xl font-black text-[#E1E0CC]">
  ...
  <div className="lg:col-span-2 rounded-xl border p-5 bg-[#101010] border-[#DEDBC8]/5 ">
   ...
   {isResearcher ? (
    ...
    <Bar dataKey="citations" fill="#DEDBC8" radius={[4, 4, 0, 0]} name="Citations" />
    </BarChart>
   ) : (
    <AreaChart data={ACADEMIC_TRENDS}>
    <defs>
     {[{ id: 'oa1', c: '#DEDBC8' }, { id: 'oa2', c: '#A09878' }, { id: 'oa3', c: '#E1E0CC' }].map((g) => (
    ...
    <Area type="monotone" dataKey="iot" stroke="#DEDBC8" fill="url(#oa1)" strokeWidth={2} name="IoT Systems" />
    <Area type="monotone" dataKey="emb" stroke="#A09878" fill="url(#oa2)" strokeWidth={2} name="Embedded C/C++" />
    <Area type="monotone" dataKey="ai" stroke="#E1E0CC" fill="url(#oa3)" strokeWidth={2} name="Hardware AI" />
    </AreaChart>
   )}
   </ResponsiveContainer>
  </div>
  ...
  <div className="rounded-xl border p-5 bg-[#101010] border-[#DEDBC8]/5 ">
   ...
   {pieData.map((f) => (
    ...
    <div className="flex items-center gap-2">
     ...
     <span className="text-gray-600 text-gray-400">{f.n}</span>
    </div>
  ...
  <div className="flex items-center justify-between p-5 border-b border-gray-200 border-[#DEDBC8]/5">
   ...
   <button className="text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors bg-gray-100 text-gray-600 hover:bg-gray-200 bg-[#0A0A0A] text-gray-400 dark:hover:bg-white/5">
  ...
  <div className="overflow-x-auto">
   ...
   {isResearcher ? (
    ...
    <thead>
     ...
     {['Paper Title', 'Journal', 'Year', 'Role', 'Citations'].map((h) => <th key={h} className="text-left px-5 py-3 text-xs font-semibold text-gray-500 text-gray-400">{h}</th>)}
     </tr>
    </thead>
    <tbody>
     {MY_PUBLICATIONS.map((p, i) => (
     <tr key={i} className="border-b border-gray-100 border-[#DEDBC8]/5 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
      <td className="px-5 py-3.5"><span className="text-xs font-semibold text-[#E1E0CC] block max-w-xs truncate">{p.title}</span></td>
      <td className="px-5 py-3.5 text-xs text-[#DEDBC8] font-medium">{p.journal}</td>
      <td className="px-5 py-3.5 text-xs text-gray-500 text-gray-400">{p.year}</td>
      <td className="px-5 py-3.5"><GlowBadge color={p.role === 'First Author' ? '#E1E0CC' : '#A09878'}>{p.role}</GlowBadge></td>
      <td className="px-5 py-3.5 text-xs font-bold text-[#E1E0CC]">{p.citations}</td>
     </tr>
     ))}
    </tbody>
    </>
    ...
    <thead>
     <tr className="border-b border-gray-200 border-[#DEDBC8]/5">
     {['Title', 'Authors', 'Year', 'Field', 'Action'].map((h) => <th key={h} className="text-left px-5 py-3 text-xs font-semibold text-gray-500 text-gray-400">{h}</th>)}
     </tr>
    </thead>
    <tbody>
     {RECOMMENDED_PAPERS.map((p, i) => (
     <tr key={i} className="border-b border-gray-100 border-[#DEDBC8]/5 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
      <td className="px-5 py-3.5"><span className="text-xs font-semibold text-[#E1E0CC] block max-w-[250px] truncate">{p.title}</span></td>
      <td className="px-5 py-3.5 text-xs text-gray-500 text-gray-400">{p.authors}</td>
      <td className="px-5 py-3.5 text-xs text-gray-500 text-gray-400">{p.year}</td>
      <td className="px-5 py-3.5"><GlowBadge color={ACADEMIC_FIELDS.find(f => f.n === p.field)?.c ?? '#DEDBC8'}>{p.field}</GlowBadge></td>
      <td className="px-5 py-3.5">
      <button className="text-xs font-semibold text-[#DEDBC8] hover:text-[#E1E0CC] transition-colors bg-[#DEDBC8]/10 px-3 py-1 rounded-md">Save</button>
      </td>
     </tr>
     ))}
    </tbody>
