...
function StatCard({ label, value, change, Icon, accent }) {
 ...
 <motion.div whileHover={{ y: -3 }} className={`p-4 ${card} flex flex-col justify-between group`}>
  ...
  {change && <span className="text-[10px] font-bold px-2 py-0.5 rounded-md" style={{ background: `${change.startsWith('+') ? '#34D399' : '#EF4444'}15`, color: change.startsWith('+') ? '#34D399' : '#EF4444' }}>{change}</span>}
  </div>
  <div>
  <h4 className="text-[10px] font-semibold uppercase tracking-wider mb-1 text-gray-400">{label}</h4>
  <div className="text-xl font-bold text-[#E1E0CC]">{value}</div>
  </div>
 </motion.div>
 );
}

function PulseDot({ color = '#A09878' }) {
 return (
...
const RESOURCE_USAGE = [
 { name: 'CPU', value: 34, color: '#DEDBC8' }, { name: 'Memory', value: 62, color: '#DEDBC8' },
 { name: 'Disk', value: 87, color: '#E1E0CC' }, { name: 'Network', value: 28, color: '#A09878' },
];
...
const RECENT_EVENTS = [
 { id: 1, msg: 'API v2.4.1 deployed successfully', time: '12 min ago', icon: CheckCircle2, color: '#A09878' },
 { id: 2, msg: 'Citation Graph API latency spike detected', time: '28 min ago', icon: AlertTriangle, color: '#E1E0CC' },
...
export default function AdminOverview() {
 ...
 <div className="p-6 space-y-5">
  ...
  <div>
   ...
   <Activity size={18} className="text-[#A09878]" />System Dashboard
   </h2>
  ...
  <StatCard label="Active Users" value="12,847" change="+8.4%" Icon={Users} accent="#DEDBC8" />
  <StatCard label="Total Requests" value="429K" change="+11%" Icon={Globe} accent="#A09878" />
  <StatCard label="Avg Latency" value="42ms" change="-8%" Icon={Clock} accent="#DEDBC8" />
  <StatCard label="Error Rate" value="0.03%" change="-15%" Icon={AlertTriangle} accent="#E1E0CC" />
  ...
  <div className={`lg:col-span-2 ${card} p-5`}>
   ...
   <h3 className="text-sm font-bold text-[#E1E0CC] flex items-center gap-2"><Globe size={14} className="text-[#DEDBC8]" /> Request Volume (24h)</h3>
   ...
   <AreaChart data={REQUEST_SERIES}>
    <defs>
    <linearGradient id="reqGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#DEDBC8" stopOpacity={0.3} /><stop offset="100%" stopColor="#DEDBC8" stopOpacity={0} /></linearGradient>
    ...
    <Area type="monotone" dataKey="requests" stroke="#DEDBC8" fill="url(#reqGrad)" strokeWidth={2} dot={false} />
  ...
  <div className={`${card} p-5`}>
   <h3 className="text-sm font-bold text-[#E1E0CC] mb-4 flex items-center gap-2"><Cpu size={14} className="text-amber-500" /> Resource Usage</h3>
   <div className="space-y-4">
   {RESOURCE_USAGE.map((r) => (
    <div key={r.name}>
    <div className="flex items-center justify-between mb-1.5">
     <span className="text-[10px] text-gray-400">{r.name}</span>
     <span className="text-[10px] font-semibold text-[#E1E0CC]">{r.value}%</span>
    </div>
  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
  <div className={`lg:col-span-2 ${card} p-5`}>
   <h3 className="text-sm font-bold text-[#E1E0CC] mb-4 flex items-center gap-2"><TrendingUp size={14} className="text-emerald-500" /> User Growth (7 days)</h3>
   ...
   <BarChart data={USER_GROWTH}>
    ...
    <Tooltip contentStyle={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 11 }} />
    <Bar dataKey="new" fill="#DEDBC8" radius={[3, 3, 0, 0]} name="New Users" />
    <Bar dataKey="active" fill="#DEDBC8" radius={[3, 3, 0, 0]} name="Active Sessions" />
   </BarChart>
   </ResponsiveContainer>
  </div>

  <div className={`${card} p-5`}>
   <h3 className="text-sm font-bold text-[#E1E0CC] mb-4 flex items-center gap-2"><Zap size={14} className="text-amber-500" /> Recent Events</h3>
   ...
   {RECENT_EVENTS.map((e) => (
    ...
    <div className="flex-1 min-w-0"><p className="text-[11px] text-[#E1E0CC] leading-snug">{e.msg}</p><span className={`text-[9px] ${subtle}`}>{e.time}</span></div>
    </div>
   ))}
   </div>
  </div>
  ...
  <ResponsiveContainer width="100%" height={200}>
   ...
   <defs>
    <linearGradient id="visGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#DEDBC8" stopOpacity={0.25} /><stop offset="100%" stopColor="#DEDBC8" stopOpacity={0} /></linearGradient>
   </defs>
   <CartesianGrid strokeDasharray="3 3" stroke="rgba(128,128,128,0.15)" />
   <XAxis dataKey="h" tick={{ fill: '#9CA3AF', fontSize: 10 }} axisLine={false} tickLine={false} interval={3} />
   ...
   <Area type="monotone" dataKey="today" stroke="#DEDBC8" fill="url(#visGrad)" strokeWidth={2} dot={false} name="Today" />
   </AreaChart>
  </ResponsiveContainer>
  </div>
  ...
  {[
   { label: 'User Management', icon: Users, color: '#DEDBC8' },
   { label: 'API Monitoring', icon: Globe, color: '#A09878' },
   { label: 'Database View', icon: Database, color: '#DEDBC8' },
   { label: 'System Settings', icon: Shield, color: '#E1E0CC' },
  ].map((q) => (
   ...
   <span className="text-xs font-semibold text-[#E1E0CC] group-hover:text-gray-600 dark:group-hover:text-blue-300 transition-colors">{q.label}</span>
