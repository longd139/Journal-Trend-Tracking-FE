...
export default function LoginPage() {
  ...
  const handleChange = (field, value) => {
    ...
    if (field === 'email' && errors.emailFormat) setErrors((e) => ({ ...e, emailFormat: false }));
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden" style={{ background: '#0B1020' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="relative z-10 w-full max-w-md mx-6">
        ...
        <div className="rounded-2xl border p-8" style={{ background: 'rgba(27,34,53,0.85)', borderColor: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(24px)' }}>
          <h2 className="text-2xl font-black text-white mb-1" style={{ fontFamily: "'Outfit', sans-serif" }}>Welcome back</h2>
          <p className="text-sm mb-6" style={{ color: '#A0AEC0' }}>Sign in to your research dashboard</p>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="email" placeholder="you@university.edu" value={form.email} onChange={(e) => handleChange('email', e.target.value)}
              ...
              {errors.emailFormat && <div className="flex items-center gap-1 mt-1.5 text-[10px] font-medium text-red-500"><AlertCircle size={10} /> Email không hợp lệ (vd: name@domain.com)</div>}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="password" placeholder="••••••••" value={form.password} onChange={(e) => handleChange('password', e.target.value)}
                  className={`w-full pl-9 pr-4 py-2.5 rounded-xl border text-sm outline-none transition-colors ${
              ...
              {errors.password && <div className="flex items-center gap-1 mt-1.5 text-[10px] font-medium text-red-500"><AlertCircle size={10} /> Vui lòng nhập mật khẩu</div>}
            </div>
            ...
            {/* Nút Submit */}
            <motion.button
              whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} type="submit" disabled={loading}
          ...
          <div className="mt-8 pt-6 border-t" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
            ...
            <div className="flex gap-2">
              <button onClick={() => autoFill('admin')} className="flex-1 py-1.5 rounded-lg text-xs font-semibold border text-red-400 border-red-400/20 hover:bg-red-400/10 transition-colors">Admin</button>
              <button onClick={() => autoFill('researcher')} className="flex-1 py-1.5 rounded-lg text-xs font-semibold border text-blue-400 border-blue-400/20 hover:bg-blue-400/10 transition-colors">Researcher</button>
              <button onClick={() => autoFill('academic')} className="flex-1 py-1.5 rounded-lg text-xs font-semibold border text-emerald-400 border-emerald-400/20 hover:bg-emerald-400/10 transition-colors">Academic</button>
            </div>
          </div>

          <div className="mt-6 pt-5 border-t text-center text-xs" style={{ borderColor: 'rgba(255,255,255,0.07)', color: '#A0AEC0' }}>
            Don't have an account? <Link to="/register" className="font-semibold" style={{ color: '#4F8CFF' }}>Register</Link>
          </div>
        </div>
