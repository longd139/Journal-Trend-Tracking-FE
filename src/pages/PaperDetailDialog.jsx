...
export function PaperDetailDialog({ paper, open, onOpenChange }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl max-h-[85vh] overflow-y-auto bg-white dark:bg-[#1B2235] border-gray-200 dark:border-white/10 text-gray-900 dark:text-white">
        {/* ── Header ── */}
        <DialogHeader>
        ...
        <div className="flex flex-wrap items-center gap-2 mt-1">
          {p.field && (
            <Badge
              variant="outline"
              className="text-[10px] font-extrabold uppercase tracking-wider px-2 py-0.5 bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/25"
            >
              {p.field}
            </Badge>
          )}
          ...
          <div className="space-y-1.5 border-t border-gray-100 dark:border-white/5 pt-4">
            ...
            {p.doi && (
              <Button
                ...
                className="flex items-center gap-2 text-xs bg-white dark:bg-white/[0.02] border-gray-200 dark:border-white/10 text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-white/5"
              >
        ...
        {/* ── Footer actions ── */}
        <DialogFooter className="border-t border-gray-100 dark:border-white/5 pt-4 mt-2 gap-2">
          {p.pdfAvailable && p.pdfUrl && (
            <Button
              type="button"
