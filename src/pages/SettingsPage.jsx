...
export default function SettingsPage() {
  ...
  // Lấy role từ sessionStorage để hiển thị data giả cho khớp
  useEffect(() => {
    const currentRole = sessionStorage.getItem('userRole') || 'academic';
    setRole(currentRole);
    
    if (currentRole === 'admin') {
      setFormData({ name: 'System Admin', email: 'admin@scitrack.com', institution: 'SCITRACK HQ', bio: 'Platform Administrator' });
    } else if (currentRole === 'researcher') {
      setFormData({ name: 'Dr. Sarah Chen', email: 'researcher@scitrack.com', institution: 'MIT', bio: 'Lead Researcher in Quantum Computing.' });
    } else {
      setFormData({ name: 'Prof. James Patel', email: 'academic@scitrack.com', institution: 'Stanford', bio: 'Professor of Computer Science.' });
    }
  }, []);
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" name="email" value={formData.email} onChange={handleChange} className="w-full pl-9 pr-4 py-2.5 rounded-xl border text-sm outline-none transition-colors focus:border-[#4F8CFF]" style={{ background: '#131A2A', borderColor: 'rgba(255,255,255,0.09)', color: '#E2E8F0' }} />
                </div>
              </div>
