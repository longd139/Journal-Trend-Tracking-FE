...
import { Home, Search, BarChart2, FileText, Users, Globe, Database, Microscope, Settings, Bell, Bookmark } from 'lucide-react';
  ...
  return (
    <aside className="w-60 flex flex-col border-r h-screen sticky top-0 shrink-0" style={{ background: '#131A2A', borderColor: 'rgba(255,255,255,0.07)' }}>
      ...
      <div className="p-4 border-t space-y-2" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
        ...
        {(() => {
          const profileConfig = {
            admin: { initials: 'AD', name: 'System Admin', desc: 'SCITRACK Administrator' },
            researcher: { initials: 'SC', name: 'Dr. Sarah Chen', desc: 'MIT · Researcher' },
            academic: { initials: 'JP', name: 'Prof. James Patel', desc: 'Stanford · Academic' }
          };
          ...
          return (
            <div className="flex items-center gap-3 p-2 rounded-lg" style={{ background: '#1B2235' }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs font-semibold text-white truncate">{currentProfile.name}</div>
                <div className="text-[10px] truncate" style={{ color: '#A0AEC0' }}>{currentProfile.desc}</div>
              </div>
              <Settings
                size={12}
                style={{ color: '#A0AEC0' }}
...
export default function DashboardLayout({ children }) {
  ...
  // LẤY ROLE THỰC TẾ TỪ sessionStorage
  const role = sessionStorage.getItem('userRole') || 'academic';

  // CẬP NHẬT TỪ ĐIỂN ĐỂ HỖ TRỢ CẢ TRANG ADMIN VÀ USER
  const titles = {
    overview: { title: role === 'admin' ? 'System Dashboard' : 'Research Overview', sub: role === 'admin' ? 'Platform health and metrics' : 'Your personalized academic intelligence dashboard' },
    ...
    bookmarks: { title: 'Saved Papers', sub: 'Your personal reading list and references' },
    users: { title: 'User Management', sub: 'Manage accounts and permissions' },
    'system-api': { title: 'API Monitoring', sub: 'Monitor platform endpoints and traffic' },
    database: { title: 'Database Control', sub: 'Manage schemas and data integrity' },
    settings: { title: 'Settings', sub: 'Manage your account profile' },
  };

  const currentHeader = titles[activeTab] || { title: 'Dashboard', sub: 'Welcome back' };
