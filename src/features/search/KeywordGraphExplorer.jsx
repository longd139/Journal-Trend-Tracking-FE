...
export default function KeywordGraphExplorer({ keyword, onKeywordClick, filters }) {
  if (!keyword?.trim()) {
    return (
      <div className="rounded-2xl border border-border bg-card-recessed p-8 flex flex-col items-center justify-center text-center min-h-[200px]">
        <div className="w-12 h-12 rounded-xl bg-primary/5 flex items-center justify-center mb-3">
          <Hash size={20} className="text-muted-foreground" />
        </div>
        <p className="text-sm text-muted-foreground">Enter a keyword to discover related trends</p>
      </div>
    );
  }
