...
function EndpointCard({ endpoint }) {
 ...
 <div className={`${card} overflow-hidden`}>
  ...
  <div className="flex items-center gap-4">
   ...
   <div className="text-xs font-bold text-[#E1E0CC] font-mono">{endpoint.latency}</div>
  ...
  {expanded && (
   ...
   <div className="p-4 grid grid-cols-2 md:grid-cols-4 gap-3">
    ...
    { l: 'Avg Latency', v: endpoint.latency, c: '#E1E0CC' },
    { l: 'Error Rate', v: endpoint.errorRate, c: '#EF4444' },
    { l: 'Success Rate', v: `${endpoint.uptime}%`, c: '#A09878' },
    ].map((s) => (
...
const ENDPOINTS = [
 { name: 'Semantic Search API', method: 'GET', path: '/api/v1/papers/search', status: 'healthy', requests: '124K', latency: '120ms', latencyValue: 120, uptime: 99.9, errorRate: '0.02%', color: '#DEDBC8' },
 { name: 'Citation Graph API', method: 'GET', path: '/api/v1/papers/search/graph', status: 'degraded', requests: '89K', latency: '850ms', latencyValue: 850, uptime: 95.5, errorRate: '1.2%', color: '#E1E0CC' },
 { name: 'User Authentication API', method: 'POST', path: '/api/auth/login', status: 'healthy', requests: '215K', latency: '45ms', latencyValue: 45, uptime: 99.99, errorRate: '0.01%', color: '#A09878' },
 { name: 'Analytics Engine API', method: 'GET', path: '/api/v1/analytics', status: 'healthy', requests: '42K', latency: '210ms', latencyValue: 210, uptime: 99.5, errorRate: '0.05%', color: '#DEDBC8' },
 { name: 'User Profile API', method: 'GET', path: '/api/users/me', status: 'healthy', requests: '310K', latency: '65ms', latencyValue: 65, uptime: 99.95, errorRate: '0.03%', color: '#DEDBC8' },
 { name: 'Paper Detail API', method: 'GET', path: '/api/v1/papers/:id', status: 'healthy', requests: '89K', latency: '90ms', latencyValue: 90, uptime: 99.8, errorRate: '0.01%', color: '#A09878' },
];
...
export default function APIMonitoring() {
 ...
 <div className="p-6 space-y-5">
  ...
  {[
   { label: 'Endpoints', value: stats.total, icon: Server, color: '#DEDBC8' },
   { label: 'Healthy', value: stats.healthy, icon: CheckCircle2, color: '#A09878' },
   { label: 'Degraded', value: stats.degraded, icon: AlertCircle, color: '#E1E0CC' },
   { label: 'Avg Uptime', value: `${stats.avgUptime}%`, icon: TrendingUp, color: '#DEDBC8' },
   { label: 'Total Requests', value: `${(stats.totalReq / 1000).toFixed(0)}K`, icon: Activity, color: '#E1E0CC' },
  ].map((s) => (
   ...
   <div><div className="text-[9px] text-gray-500 uppercase tracking-wider">{s.label}</div><div className="text-sm font-black text-[#E1E0CC] font-mono">{s.value}</div></div>
   </motion.div>
  ))}
  </div>

  <div className="space-y-3">
  <h3 className="text-sm font-bold text-[#E1E0CC] flex items-center gap-2"><Globe size={14} className={subtle} /> API Endpoints</h3>
